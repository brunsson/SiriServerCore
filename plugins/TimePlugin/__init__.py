...
localizations = {
    ...
    "failure":
    {
      ...
      "fr-FR": [u"Je ne peux pas donner l'heure pour cette demande"]
    }
}
def getNameFromGoogle(request):
    try:
        ...
        location = root["results"][0][u"formatted_address"]
        print location
        return location
    except:
        return None
...
class currentTime(Plugin):
    ...
    @register("fr-FR", u"((Quel.*heur.*à)|(.L'heure*)) (?P<loc>[\w ]+)")
    def currentTimeIn(self, speech, language, matchedRegex):
        
        self.showWait(language)
        
        location = matchedRegex.group(u"loc")
        ...
        query = "select name from geo.places.belongtos where member_woeid in (select woeid from geo.places where text=\"{0}\") and placetype=31".format(location.encode("utf-8"))
        request = "http://query.yahooapis.com/v1/public/yql?q={0}&format=json&callback=".format(urllib.quote(query.encode("utf-8")))
        timeZoneId = None
        try:
            result = getWebsite(request, timeout=5)
            root = json.loads(result)
